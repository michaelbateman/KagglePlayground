...
N = get_matrix_from_data('test.csv')
...
num_train_samples =  len(M[:,0]) #int(sys.argv[1]) # len(M[:,0]) / 2 - 1
num_test_samples = len(N[:,0])
my_degree = 3#int(sys.argv[2])

num_estimators = 5000
num_units = 100
y_train = M[0:num_train_samples,0:1]
X_train = M[0:num_train_samples,1:]


#Use this for running on kaggle data
X_test = N[0:num_test_samples,:]
...
clf_list = [RandomForest]#,  NeuralNetwork, SupportVector]
#print RandomForest
...
for clf in clf_list:
	...
	c
	# Use this to check accuracy on reserved portion of training set
	#X_test = M[num_samples:2*num_samples,1:]
	#y_correct =  M[num_samples:2*num_samples,0:1]
	
	#Use this to check accuracy on training set
	#X_test = X_train
...
image_id = [0]*num_test_samples
for j in range(0, num_test_samples):
	image_id[j] = j+1
	#print j, len(y_test), len(y_test_vector[RandomForest])
	
	if only_RF == 'yes':
		y_test[j] = y_test_vector[RandomForest][j]
	
	elif y_test_vector[RandomForest][j] == y_test_vector[NeuralNetwork][j] and ensemble == 'on':
		y_test[j] = y_test_vector[RandomForest][j]
	else:
...
#print y_test
prediction_file = open('RFonlykaggle.csv', 'wb')
#prediction_file.write('HEADER\n')
#for j in range(0, num_test_samples):
	#prediction_file.write(str(y_test[j])+'\n')
	
file_object = csv.writer(prediction_file)
file_object.writerow(['ImageId','Label'])
file_object.writerows(zip(image_id,y_test))
